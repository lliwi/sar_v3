...
@admin_required
def create_backup():
    """Create a complete backup of the system including database and configuration"""
    try:
        ...
        current_app.logger.info("Starting database export...")
        database_dir = os.path.join(temp_dir, 'database')
        os.makedirs(database_dir, exist_ok=True)
        
        # Get all model classes
        from app.models import (User, Role, Folder, ADGroup, FolderPermission, 
                               PermissionRequest, AuditEvent, Task, UserADGroupMembership, 
                               UserFolderPermission, AdminNotification)
        ...
        for table_name, model_class in models:
            try:
                ...
                backup_data['database_tables'].append({
                    'table': table_name,
        ...
        # 2. Copy .env file with configurations
        current_app.logger.info("Including .env configuration file...")
        env_source = os.path.join('/home/llibert/Documents/development/flask/SAR_v3', '.env')
        if os.path.exists(env_source):
            env_dest = os.path.join(temp_dir, '.env')
            shutil.copy2(env_source, env_dest)
            backup_data['files_included'].append('.env')
            current_app.logger.info("Environment file included in backup")
        else:
            current_app.logger.warning(".env file not found")
        
        # 3. Include basic database information
        current_app.logger.info("Including database information...")
        try:
            schema_file = os.path.join(temp_dir, 'database_info.txt')
            
            with open(schema_file, 'w') as f:
                f.write("SAR System Database Information\n")
                f.write(f"Generated on: {datetime.now().isoformat()}\n")
                f.write(f"Created by: {current_user.username}\n\n")
                f.write("Database contains the following tables:\n")
                
                for table_name, model_class in models:
                    record_count = model_class.query.count()
                    f.write(f"- {table_name}: {record_count} records\n")
            
            backup_data['files_included'].append('database_info.txt')
            current_app.logger.info("Database information included in backup")
            
        except Exception as e:
            current_app.logger.warning(f"Could not export database info: {str(e)}")
        
        # 4. Include system information and metadata
        current_app.logger.info("Including system metadata...")
        metadata = {
            'system_info': {
                'application': 'SAR v3 - Sistema de Gestión de Permisos',
                'version': '3.0',
                'backup_date': datetime.now().isoformat(),
                'created_by': {
            ...
            'database_info': {
                ...
                'total_records': sum(item.get('records', 0) for item in backup_data['database_tables']),
                'tables': backup_data['database_tables']
            },
            'restoration_instructions': {
                'steps': [
                    '1. Restore database structure using schema.sql',
                    '2. Import data from JSON files in database/ directory',
                    '3. Configure environment using the included .env file',
                    '4. Restart the application services',
                    '5. Verify system functionality and data integrity'
                ],
                'requirements': [
                    'PostgreSQL database server',
                    'Python 3.8+ with Flask dependencies',
                    'Docker and Docker Compose (for containerized deployment)',
                    'Access to LDAP/Active Directory server (configured in .env)',
                    'SMTP server for notifications (configured in .env)'
                ]
            }
        }
        ...
        backup_data['files_included'].append('backup_metadata.json')
        
        # 5. Create restoration script
        current_app.logger.info("Creating restoration script...")
        restore_script = '''#!/bin/bash
# SAR System Backup Restoration Script
# This script helps restore the SAR system from a backup

echo "Starting SAR System restoration..."

# Check if running as root/admin
if [[ $EUID -ne 0 ]]; then
   echo "This script should be run with administrator privileges for database operations"
fi
...
echo "Step 1: Restoring database schema..."
if [ -f "$BACKUP_DIR/schema.sql" ]; then
    psql -U $DB_USER -d $DB_NAME -f "$BACKUP_DIR/schema.sql"
...
# 3. Instructions for data restoration
echo "Step 3: Data restoration (manual process)"
echo "The database table data is available in JSON format in the 'database/' directory"
echo "You will need to import these files using your application's data import functionality"
echo ""
echo "Available data files:"
ls -la "$BACKUP_DIR/database/"

echo ""
echo "Restoration preparation completed!"
echo "Please review backup_metadata.json for detailed restoration instructions"
'''
        
        restore_script_file = os.path.join(temp_dir, 'restore.sh')
        with open(restore_script_file, 'w') as f:
            f.write(restore_script)
        os.chmod(restore_script_file, 0o755)  # Make executable
        backup_data['files_included'].append('restore.sh')
        ...
        AuditEvent.log_event(
            ...
            resource_type='system',
            resource_id=None,
            description='Backup completo del sistema creado',
            metadata={
                'backup_file': zip_filename,
                'database_tables': len(backup_data['database_tables']),
                'total_records': sum(item.get('records', 0) for item in backup_data['database_tables']),
                'files_included': backup_data['files_included']
            },
            ip_address=request.remote_addr,
            user_agent=request.headers.get('User-Agent')
        )
        ...
        import traceback
        current_app.logger.error(f"Backup error traceback: {traceback.format_exc()}")
